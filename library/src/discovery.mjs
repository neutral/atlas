...
export function findAtlasRoot(start, sourceFs = fs) {
  let current = path.resolve(start);
  try { const m=sourceFs.lstatSync(current); if (m.isFile()||m.isSymbolicLink()) current=path.dirname(current); }
  catch (error) {
  ...
  while (true) {
    ...
    try { const m=sourceFs.lstatSync(candidate); if (m.isFile()||m.isSymbolicLink()) return current; }
    catch (error) {
...
export function discover(root, diagnostics, sourceFs = fs) {
  const state={root,fs:sourceFs,complete:true,files:[],maps:[],pointRecords:[],checks:[],publications:[],pointIdentities:new Map(),atlas:structural(path.join(root,'atlas.md'),'atlas'),catalog:structural(path.join(root,'catalog.json'),'catalog'),connections:structural(path.join(root,'connections.json'),'connections')};
...
function discoverDirectory(state,directory,isRoot,diagnostics) {
  ...
  for(const entry of entries){if((!isRoot&&entry.name==='map.md')||entry.name==='points')continue; if(isRoot&&['atlas.md','catalog.json','connections.json','.checks','.publication','map.md'].includes(entry.name))continue; if(IGNORED_DIRECTORIES.has(entry.name))continue; const target=path.join(directory,entry.name);
