...
export function sortedEntries(directory,sourceFs=fs){return sourceFs.readdirSync(directory,{withFileTypes:true}).sort((a,b)=>compareCodePoints(a.name,b.name));}
...
export function exactCase(root,target,sourceFs=fs){const rel=path.relative(root,target);let current=root;for(const segment of rel.split(path.sep).filter(Boolean)){const entries=sourceFs.readdirSync(current);if(!entries.includes(segment))return false;current=path.join(current,segment);}return true;}
export function pathCaseOrNormalizationMismatch(root,target,sourceFs=fs){const rel=path.relative(root,target);let current=root;for(const segment of rel.split(path.sep).filter(Boolean)){let entries;try{entries=sourceFs.readdirSync(current);}catch{return false;}if(entries.includes(segment)){current=path.join(current,segment);continue;}const key=segment.normalize('NFC').toLowerCase(),match=entries.find(entry=>entry.normalize('NFC').toLowerCase()===key);if(match)return true;return false;}return false;}
export function pathContainsSymlink(root,target,sourceFs=fs){const rel=path.relative(root,target);let current=root;for(const segment of rel.split(path.sep).filter(Boolean)){current=path.join(current,segment);let m;try{m=sourceFs.lstatSync(current);}catch{return false;}if(m.isSymbolicLink())return true;}return false;}
