...
The reference implementation identifies itself as version `0.9.0` with status `stable`. Its default specification revision is `0.9.0`; callers MAY supply a more specific immutable revision identifier. A version label alone does not identify an exact pre-1.0 contract; reproducible claims identify the immutable specification revision and processor revision.
...
| `atlas.markdown.title` | error | The document lacks one non-blank top-level H1. |
| `atlas.markdown.summary` | error | The H1 lacks an immediately following non-blank paragraph. |
| `atlas.markdown.section` | error | A required meaning section or its substantive text is missing. |
| `atlas.markdown.association` | error | A meaning entry is malformed, misplaced, repeated, or names an undeclared object. |

### Global declarations

- `atlas.global.missing` (error): a required root JSON file is absent.
- `atlas.global.invalid-file` (error): a required global path is not a regular file.
- `atlas.global.invalid-json` (error): global JSON violates strict parsing.
- `atlas.global.schema` (error): a global object fails its authored schema.
- `atlas.global.owner` (error): a declaration has no unique discovered owner.
- `atlas.global.duplicate` (error): a catalog identity or exact owner repeats.
- `atlas.catalog.check-missing` (error): a discovered Check lacks its registration.
- `atlas.connection.duplicate-id` (error): a connection ID repeats across arrays.

Global diagnostics identify the actual JSON file and authored pointer. Invalid
local headers or global declarations stop dependent assembly; validators MUST
NOT manufacture missing-prose failures from unavailable metadata.
