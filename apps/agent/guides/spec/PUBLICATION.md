...
A publication profile is one regular Markdown file directly inside the optional root `.publication/` directory. The directory MUST NOT contain symbolic links, nested directories, or non-Markdown files. The front-matter `id` owns profile identity; renaming the file preserves that identity.
...
Profile front matter conforms to `urn:atlas:schema:publication:1` and requires `type`, `id`, and `selection`. The H1 and opening paragraph supply title and summary under the shared Format rules. The body explains the intended audience or editorial boundary. It does not select material.
...
A profile selects logical source units across Markdown, catalogs, and connections.
It MUST NOT select an entire global JSON file. A selected Map includes its Areas;
a selected Point record includes only its local associations; a selected anchor
includes its classifications, review, and outgoing relations. Selected Checks
include their catalog metadata. A selected Resource includes its registration
and root-owned description even when the root record is unselected.

Selecting the root excludes unselected Resource description sections from its
published body. Global storage MUST NOT broaden any selection. Build and serving
implementations enforce this boundary in rendered pages, search, and machine data.

## Processing and normalized representation

Structural processing discovers every direct profile file, applies the publication schema,  rejects invalid entries or duplicate profile ids.
...
Normalized profiles appear in `publicationProfiles` under `urn:atlas:schema:normalized:2`. An invalid or incomplete result exposes no normalized model.
