...
Atlas Portal supplies a reusable read-only publication reader driven by Atlas source, a publication profile, and an explicit portal configuration. Atlas Library supplies validation and source reading through public APIs. Atlas Portal provides compilation, routes, layout, search, responsive behavior, and static output. Successful builds establish behavior for the tested inputs; they do not establish universal corpus coverage or reader benefit.
...
```text
Atlas source
  -> Atlas Library opened view and normalized Atlas model
  -> publication-profile selection
  -> selected source reading through Atlas Library
  -> portal navigation and route data
  -> Astro reader
  -> static site
```

The Atlas contracts own meaning and semantic constraints. Atlas Library exposes validation, normalized output, and source reading. The portal compiler owns publication selection and reader derivation. Astro owns routes, HTML, styles, assets, development serving, and static generation.

The compiler opens one reusable view through the `atlas-reference-validator` package's public `openAtlas` export. It requires complete valid normalized output and owns a separate mutable copy for publication projection. Selected local Resources use the view's `readSource` operation with explicit allowed roots and byte limits. The compiler does not copy private parsing or source-reading logic.
...
The opened local view and selected source observations remain build inputs. Atlas Portal does not export the full input inventory, captured draft bytes, or a local editing or evaluation service. Source reads and the normalized observation can occur at different times; this pipeline does not establish an atomic filesystem snapshot.
