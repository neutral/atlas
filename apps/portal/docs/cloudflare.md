...
Follow [Build from source](../README.md#build-from-source) for the optional
Cloudflare helpers. The frozen workspace installation supplies Wrangler 4.125.0.
The `dev` and `deploy` helpers invoke `pnpm exec wrangler` and require pnpm
11.22.0 on `PATH`. Run this guide's commands from the source repository root:

```sh
pnpm --version
pnpm --filter atlas-portal exec wrangler --version
```

Ordinary npm installation and `atlas export` require no Wrangler or Cloudflare
account.

Create a project-owned `wrangler.jsonc` using the configuration above. Give it
the intended Worker name. With `assets.directory` set to `./dist`, pass that
project's absolute `dist` path as `--out-dir`. Keep the configuration and
generated output in the target project. The helpers default to
`apps/portal/wrangler.jsonc` and `apps/portal/dist/` in the source checkout when
explicit paths are absent.

Installed static-build qualification does not qualify this optional tool installation, Cloudflare account state, authentication, or a deployment. The source repository separately tests its pinned Wrangler setup and deployment dry run. A dry run establishes packaging for its exact inputs; it does not establish an uploaded site or account access policy.
...
```text
pnpm --filter atlas-portal exec wrangler login
pnpm --filter atlas-portal exec wrangler whoami
```
...
```text
pnpm --filter atlas-portal exec node src/cloudflare/cli.mjs deploy \
  --atlas "/absolute/path/to/project/atlas" \
  --profile internal-view \
  --portal-config "/absolute/path/to/project/portal.json" \
  --config "/absolute/path/to/project/wrangler.jsonc" \
