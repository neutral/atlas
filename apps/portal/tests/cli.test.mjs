...
test('package symlink entrypoints execute both advertised CLI commands', t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'portal-installed-bin-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  fs.symlinkSync(applicationRoot, path.join(directory, 'atlas-portal'), 'dir');
  for (const [relative, expected] of [['src/cli/index.mjs', /atlas-portal build/u], ['src/cloudflare/cli.mjs', /atlas-portal-cloudflare/u]]) {
    const result = spawnSync(process.execPath, [path.join(directory, 'atlas-portal', relative), '--help'], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
    assert.match(result.stdout, expected);
  }
});

test('CLI imports remain inert with absent or nonexistent process entrypoints', () => {
  for (const setup of ['delete process.argv[1]', `process.argv[1] = ${JSON.stringify(path.join(applicationRoot, 'not-an-entrypoint.mjs'))}`]) {
    const imports = ['src/cli/index.mjs', 'src/cloudflare/cli.mjs'].map(relative => `await import(${JSON.stringify(pathToFileURL(path.join(applicationRoot, relative)).href)})`).join(';');
    const result = spawnSync(process.execPath, ['--input-type=module', '-e', `${setup};${imports}`], { encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
...
test('concurrent CLI invocations isolate their corpus and clean failed-build caches', { skip: process.platform === 'win32' }, async (t) => {
  ...
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const isolated = path.join(directory, 'application');
  const isolatedCli = path.join(isolated, 'src/cli/index.mjs');
  fs.mkdirSync(path.dirname(isolatedCli), { recursive: true });
  fs.copyFileSync(cli, isolatedCli);
  fs.symlinkSync(path.join(applicationRoot, 'src/core'), path.join(isolated, 'src/core'));
  const astro = path.join(isolated, 'node_modules/astro');
  fs.mkdirSync(astro, { recursive: true });
  fs.writeFileSync(path.join(astro, 'package.json'), JSON.stringify({ name: 'astro', exports: { './package.json': './package.json' }, bin: { astro: './astro.cjs' } }));
  // Substitute only the installed Astro dependency. Both CLI invocations compile real Atlas input.
  fs.writeFileSync(path.join(astro, 'astro.cjs'), `#!/usr/bin/env node
...
`, { mode: 0o755 });
  async function invoke(name) {
    ...
    const child = spawn(process.execPath, [isolatedCli, 'build', '--atlas', fixture, '--profile', 'public', '--portal-config', config], {
      env: { ...process.env, PORTAL_TEST_BARRIER: directory, PORTAL_TEST_ID: name },
