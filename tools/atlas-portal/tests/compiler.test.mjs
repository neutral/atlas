...
test('compiler projects one explicit publication profile', async () => {
  ...
  assert.equal(corpus.specificationRevision, '0.8.0');
...
test('Astro builds a complete static reader from the fixture', async (t) => {
  ...
  pointMetadata.content = [{ resource: 'overview' }];
  ...
  const otherMap = fs.readFileSync(path.join(sourceDirectory, 'maps/one/map.md'), 'utf8')
    .replace('"id": "one"', '"id": "two"').replace('"title": "One"', '"title": "Two"')
  ...
  const contextMetadata = {
    ...
    summary: 'The same service boundary has a separate consequence in Two.',
    areas: [{ area: 'scope', context: 'This boundary constrains the second Map scope.' }],
    references: [{ resource: 'overview', role: 'supporting' }],
  };
  ...
  for (let index = 2; index <= 18; index += 1) {
    ...
    const metadata = { ...pointMetadata, id, title: `Boundary ${index}` };
  ...
  const mapMetadata = JSON.parse(fs.readFileSync(path.join(sourceDirectory, 'maps/one/map.md'), 'utf8').split('---')[1]);
  ...
  assert.equal(mapPage.includes(mapMetadata.areas[0].question), false);
  for (const [page, question] of [[mapPage, mapMetadata.question], [areaPage, mapMetadata.areas[0].question]]) {
    assert.equal(page.includes(question), false);
...
test('Resource roots admit ordinary dot-prefixed names and require explicit outside access', async (t) => {
  ...
  const atlasPath = path.join(source, 'atlas.md');
